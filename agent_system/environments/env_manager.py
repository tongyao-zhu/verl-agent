...
def make_envs(config):
    ...
    # For validation environments, check if Pass@k is enabled via val_kwargs.n
    val_group_n = 1  # default
    if hasattr(config, 'actor_rollout_ref') and hasattr(config.actor_rollout_ref, 'rollout'):
        ...
        if val_n > 1:
            val_group_n = val_n
            print(f"🎯 Creating validation environments with group_n={val_group_n} for Pass@{val_n}")
    ...
    if "gym_cards" in config.env.env_name.lower():
        ...
        import time
        time.sleep((config.data.train_batch_size * group_n + config.data.val_batch_size * val_group_n) * 0.1) # wait for the envs to be ready
        return envs, val_envs
